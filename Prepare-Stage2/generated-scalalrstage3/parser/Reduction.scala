...
def reduction(dol$START:  org.sufrin.scalalr.SourceLocation, dol$END:  org.sufrin.scalalr.SourceLocation, n: Int): PartialFunction[List[Any], Any] = n match {
 ...
 /* Rule: Rule = LHS `=` S_4 RHS { Rule($LHS, $RHS, $START) }  */
 case 21 => 
  { case List(dol$LHS: TypedNonterminal, _, dol$S_4: Option[_], dol$RHS: List[Production @unchecked]) => 
 ...
 /* RHS: List[Production] = S_5 { $S_5 }  */
 case 26 => 
  { case List(dol$S_5: List[Production @unchecked]) =>  dol$S_5 } 
 ...
 /* S_5_L: List[Production] = Production { List($Production) }  */
 case 59 => 
  { case List(dol$Production: Production) =>  List(dol$Production) } 
 /* S_5_L: List[Production] = S_5_L `|` Production { $Production :: $S_5_L }  */
 case 60 => 
  { case List(dol$S_5_L: List[Production @unchecked], _, dol$Production: Production) => 
        dol$Production :: dol$S_5_L
  }
 /* S_5: List[Production] = S_5_L { $S_5_L.reverse }  */
 case 61 => 
  { case List(dol$S_5_L: List[Production @unchecked]) =>  dol$S_5_L.reverse } 
