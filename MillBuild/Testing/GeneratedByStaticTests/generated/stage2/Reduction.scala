...
def reduction(dol$START:  org.sufrin.scalalr.SourceLocation, dol$END:  org.sufrin.scalalr.SourceLocation, n: Int): PartialFunction[List[Any], Any] = n match {
 /* Notation: Notation = Prefix `%rules` firstInclude: INCLUDE Rules OPTSEPARATOR secondInclude: INCLUDE {  $Prefix.copy(theRules = $Rules.reverse, theRulesInclude = $firstInclude++"\n\n"++$secondInclude)  }  */
 case 1 => 
  { case List(dol$Prefix: Notation,  _ , dol$firstInclude: String, dol$Rules: List[Rule @unchecked],  _ , dol$secondInclude: String) => 
        dol$Prefix.copy(theRules = dol$Rules.reverse, theRulesInclude = dol$firstInclude++"\n\n"++dol$secondInclude)
  }
 ...
 /* Prefix: Notation = p: Prefix `%infer` ID {  $p.withInferDeclaration($ID.toString) }  */
 case 13 => 
  { case List(dol$p: Notation, _, dol$ID: org.sufrin.scalalr.stage2.AST.Name) => 
        dol$p.withInferDeclaration(dol$ID.toString)
  }
