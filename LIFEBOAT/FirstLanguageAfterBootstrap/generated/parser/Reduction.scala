...
def parsetreereduction(dol$START:  org.sufrin.scalalr.SourceLocation, dol$END:  org.sufrin.scalalr.SourceLocation, n: Int): PartialFunction[List[Any], Any] = n match {
 case 1 => 
  { case trees$trees => PARSETREE("""Types: List[Type] = Type { { List($Type) }}""", 1, trees$trees ) }
 case 2 => 
  { case trees$trees => PARSETREE("""Types: List[Type] = Type "," Types { { $Type :: $Types }}""", 2, trees$trees ) }
 case 3 => 
  { case trees$trees => PARSETREE("""Type: Type = ID { { Type($ID, Nil, $START) }}""", 3, trees$trees ) }
 case 4 => 
  { case trees$trees => PARSETREE("""Type: Type = ID "[" Types "]" { { Type($ID, $Types, $START) }}""", 4, trees$trees ) }
 case 5 => 
  { case trees$trees => PARSETREE("""Type: Type = "(" Types ")" { { makeTupleType($Types, $START) }}""", 5, trees$trees ) }
 case 6 => 
  { case trees$trees => PARSETREE("""Type: Type = "(" ")" { { Type("Unit", Nil, $START) }}""", 6, trees$trees ) }
 case 7 => 
  { case trees$trees => PARSETREE("""Precedence: Option[Terminal] =  { { None }}""", 7, trees$trees ) }
 case 8 => 
  { case trees$trees => PARSETREE("""Precedence: Option[Terminal] = "%prec" ID { { Some(new Terminal($ID)) }}""", 8, trees$trees ) }
 case 9 => 
  { case trees$trees => PARSETREE("""Action: Option[Expression] =  { { None }}""", 9, trees$trees ) }
 case 10 => 
  { case trees$trees => PARSETREE("""Action: Option[Expression] = CODE { { Some($CODE) }}""", 10, trees$trees ) }
 case 11 => 
  { case trees$trees => PARSETREE("""NamedField: NamedField = ID { { NamedField(theName = None, fieldSymbol = $ID, $START) }}""", 11, trees$trees ) }
 case 12 => 
  { case trees$trees => PARSETREE("""NamedField: NamedField = theName: ID ":" fieldSymbol: ID { { NamedField(Some($theName), $fieldSymbol, $START) }}""", 12, trees$trees ) }
 case 13 => 
  { case trees$trees => PARSETREE("""NamedFields: List[NamedField] = "%empty" { { Nil }}""", 13, trees$trees ) }
 case 14 => 
  { case trees$trees => PARSETREE("""NamedFields: List[NamedField] = NamedField { { List($NamedField) }}""", 14, trees$trees ) }
 case 15 => 
  { case trees$trees => PARSETREE("""NamedFields: List[NamedField] = NamedField NamedFields { { $NamedField :: $NamedFields }}""", 15, trees$trees ) }
 case 16 => 
  { case trees$trees => PARSETREE("""Production: Production = NamedFields Action Precedence { { Production($NamedFields, $Action, $Precedence, $START) }}""", 16, trees$trees ) }
 case 17 => 
  { case trees$trees => PARSETREE("""RHS: List[Production] = Production { { List($Production)   }}""", 17, trees$trees ) }
 case 18 => 
  { case trees$trees => PARSETREE("""RHS: List[Production] = Production "|" RHS { { $Production :: $RHS }}""", 18, trees$trees ) }
 case 19 => 
  { case trees$trees => PARSETREE("""LHS: TypedNonterminal = ID ":" Type { {  TypedNonterminal($ID, $Type, $START) }}""", 19, trees$trees ) }
 case 20 => 
  { case trees$trees => PARSETREE("""LHS: TypedNonterminal = ID { {  TypedNonterminal($ID, Untyped, $START) }}""", 20, trees$trees ) }
 case 21 => 
 ...
 case 25 => 
  { case trees$trees => PARSETREE("""Rules: List[Rule] = Rules ";" Rule { { $Rule :: $Rules }}""", 25, trees$trees ) }
 case 26 => 
  { case trees$trees => PARSETREE("""Tables: String =  { { "lalr" }}""", 26, trees$trees ) }
 case 27 => 
  { case trees$trees => PARSETREE("""Tables: String = "%tables" ID { { mkTableType($ID) }}""", 27, trees$trees ) }
 case 28 => 
  { case trees$trees => PARSETREE("""TypedTerminal: TypedTerminal = ID ":" Type { {  TypedTerminal($ID, $Type, $START)   }}""", 28, trees$trees ) }
 case 29 => 
  { case trees$trees => PARSETREE("""TypedTerminal: TypedTerminal = ID "(" Type ")" { {  TypedTerminal($ID, $Type, $START) }}""", 29, trees$trees ) }
 case 30 => 
  { case trees$trees => PARSETREE("""TypedTerminal: TypedTerminal = ID { {  TypedTerminal($ID, Untyped, $START) }}""", 30, trees$trees ) }
 case 31 => 
  { case trees$trees => PARSETREE("""TypedTerminals: List[TypedTerminal] = TypedTerminal { { List($TypedTerminal) }}""", 31, trees$trees ) }
 case 32 => 
  { case trees$trees => PARSETREE("""TypedTerminals: List[TypedTerminal] = TypedTerminal TypedTerminals { { $TypedTerminal :: $TypedTerminals }}""", 32, trees$trees ) }
 case 33 => 
  { case trees$trees => PARSETREE("""TokenSpec: TokenSpec = "%left" TypedTerminals { { Left($TypedTerminals) }}""", 33, trees$trees ) }
 case 34 => 
  { case trees$trees => PARSETREE("""TokenSpec: TokenSpec = "%right" TypedTerminals { { Right($TypedTerminals) }}""", 34, trees$trees ) }
 case 35 => 
  { case trees$trees => PARSETREE("""TokenSpec: TokenSpec = "%non" TypedTerminals { { Nonassoc($TypedTerminals) }}""", 35, trees$trees ) }
 case 36 => 
  { case trees$trees => PARSETREE("""TokenSpec: TokenSpec = "%token" TypedTerminals { { Tokens($TypedTerminals) }}""", 36, trees$trees ) }
 case 37 => 
  { case trees$trees => PARSETREE("""Tokens: List[TokenSpec] =  { { Nil }}""", 37, trees$trees ) }
 case 38 => 
  { case trees$trees => PARSETREE("""Tokens: List[TokenSpec] = TokenSpec Tokens { { $TokenSpec :: $Tokens }}""", 38, trees$trees ) }
 case 39 => 
  { case trees$trees => PARSETREE("""OptInclude: String = "%empty" { { "" }}""", 39, trees$trees ) }
 case 40 => 
  { case trees$trees => PARSETREE("""OptInclude: String = "%include" CODE { { $CODE }}""", 40, trees$trees ) }
 case 41 => 
  { case trees$trees => PARSETREE("""Notation: Notation = "%notation" theName: ID "%package" thePackage: ID "%path" thePath: ID Tables tokensInclude: OptInclude Tokens "%rules" rulesInclude: OptInclude Rules OptSemicolon { { Notation($thePackage, $theName, $thePath, $Tables, "Scanner", Type("Token", Nil, $START), $Tokens, $Rules, $tokensInclude, $rulesInclude) }}""", 41, trees$trees ) }
 case 42 => 
  { case trees$trees => PARSETREE("""command: Unit = Notation { { translate($Notation) }}""", 42, trees$trees ) }
 }
