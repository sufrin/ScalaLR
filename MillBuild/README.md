...
1. Tokens (terminal symbols) *must* be specified. Each that carries an irredundant value
   must have the type of that value specified. The rest are considered
   "punctuation".

2. Shift-reduce conflicts, a form of ambiguity,  can be resolved by specifying the
   associativity and precedence of (terminal) symbols, as in Bison. Lower
   priority operators appear than those of higher priority.
````
%rules
...
## Alternative Result Notations [mid-June 2026]
...
### Constructor-only specification of results

An even simpler and (we think) less error-prone notation for result expressions 
exploits the fact that scala methods can be called with "keyword" parameters.
It is enabled only when

   %infer results

has appeared in the prologue (before the `%rules`) and *supplements*
the existing `=>` scheme.

#### Example
   Suppose our expression abstract syntax has been simplified to:
```scala
      trait Expr
...
```
then a concise way of expressing `Expr` yielding productions is to express result
expressions using only constructors is:
````
expr: Expr = ID                    => Id               
           ...
           | "(" expr ")"          => Bra      
````
The code generator transforms this into
````
expr: Expr = ID                    {Id($ID)}               
...
#### Result Expression transformation rules
1.  When `C` can be interpreted as a result-constructing expression (Constructor),
then rules of the form

         lhs: Type = ... l1: S1 ... ln: Sn => C

Are transformed so that the `C` is invoked with named arguments that match (what are assumed to be) 
its parameter names.

         lhs: Type = ... l1: S1 ... ln: Sn { C(l1=$l1, ... ln=$ln) }

2. If `TT` is the sole value-carrying symbol of a production, then it isn't necessary to label it; so
result expressions of the form

         lhs: Type = ... TT ... => C

then the production is transformed  into

         lhs: Type = ... TT ... => C($TT)


#### Notes
1. We also have at the back of our mind the derivation of
unparsers (linearisers) more or less directly from constructor-only definitions.

2. Our intention is eventually to be able to dispense
...
## Gotchas
1. **Error recovery** is not completely implemented. But coherent reports locating the
   first syntax error in a file/session will nevertheless be made (from the `Pull` parser), 
   providing its `locateError` feature is `true` (the usual situation). Placing  
   "error productions" judiciously in definitions of nonterminals (sometimes) makes it possible
   for the name of the failing nonterminal to be shown in the report. Such productions take the
   *exact* form:
   ```scala
       error => ()
   ```
   
   On the other hand, it is straightforward to construct a Read-Eval-Print-type interface 
   that appears to recover from
   syntax errors: see `Examples/TinyFun/src/runtinyfun` for an example.

   **Error recovery implementation details** are evolving, and documented in the source 
   text of the `LRParser` module.
   

2. **Inadvertent Scala type errors in result Expressions** are
...
for little mistakes (omitting `$`), but ideally this would
be done by an "onboard" type-checker. 
Happily, with a modern IDE, supported by `mill` the
workflow involved in getting this aspect of a grammar correct is not too tedious. 
...
it is evaluated when the production has been parsed.
   ...
   has a side-effect -- the pretty-printing of the result `expr`. 
   Its result is the value `Void`, which is of type
   `Void`, as defined in the parse-time inclusion.
   
       { $expr.prettyPrint(); Void }
...
````scalalr
      ...
      /**
        The following "parse-time" %include becomes part of the Scala source file 
        `Reduction.scala` generated by ScalaLR  that defines the correspondence 
        between productions and their result expressions. 
...
BS: April 29th, May 13th 2026, June 12th, July 6th 2026
