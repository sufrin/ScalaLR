...
def parsetreereduction(dol$START:  org.sufrin.scalalr.SourceLocation, dol$END:  org.sufrin.scalalr.SourceLocation, n: Int): PartialFunction[List[Any], Any] = n match {
 ...
 case 2 => 
  { case trees$trees => PARSETREE("""loop: Unit = loop command NL {  print("> "); System.out.flush() }""", 2, trees$trees ) }
 case 3 => 
 ...
 case 6 => 
  { case trees$trees => PARSETREE("""command: Unit = LOG {  }""", 6, trees$trees ) }
 case 7 => 
  { case trees$trees => PARSETREE("""command: Unit =  {  System.exit(0) }""", 7, trees$trees ) }
 case 8 => 
  { case trees$trees => PARSETREE("""expr: Expr = ID {  Id($ID, $START) }""", 8, trees$trees ) }
 case 9 => 
  { case trees$trees => PARSETREE("""expr: Expr = NUM {  Num($NUM.toDouble, $START) }""", 9, trees$trees ) }
 case 10 => 
  { case trees$trees => PARSETREE("""expr: Expr = ID "=" expr {  Assign($ID, $expr, $START) }""", 10, trees$trees ) }
 case 11 => 
 ...
 case 15 => 
  { case trees$trees => PARSETREE("""expr: Expr = l: expr "-" r: expr {  Binop("-", $l, $r, $START) }""", 15, trees$trees ) }
 case 16 => 
  { case trees$trees => PARSETREE("""expr: Expr = "(" expr ")" {  $expr }""", 16, trees$trees ) }
 case 17 => 
  { case trees$trees => PARSETREE("""expr: Expr = "(" error ")" {  Num(1.0/0.0, $START) }""", 17, trees$trees ) }
 case 18 => 
  { case trees$trees => PARSETREE("""expr: Expr = ID "(" exprs ")" {  Apply($ID, $exprs.toList, $START) }""", 18, trees$trees ) }
 case 19 => 
  { case trees$trees => PARSETREE("""exprs: RevSeq[Expr] = expr {  RevSeq($expr) }""", 19, trees$trees ) }
 case 20 => 
  { case trees$trees => PARSETREE("""exprs: RevSeq[Expr] = exprs "," expr {  $exprs :+ $expr }""", 20, trees$trees ) }
 }
