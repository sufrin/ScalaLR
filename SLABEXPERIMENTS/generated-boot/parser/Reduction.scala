...
def reduction(dol$START:  org.sufrin.scalalr.SourceLocation, dol$END:  org.sufrin.scalalr.SourceLocation, n: Int): PartialFunction[List[Any], Any] = n match {
 ...
 case 2 => 
  { case List(_, dol$theName: String, _, dol$thePackage: String, dol$thePath: String, dol$OptDialects: (String,String), dol$Tables: String, dol$tokensInclude: String, dol$Tokens: List[TokenSpec @unchecked], dol$RULES: Unit, dol$rulesInclude: String, _, dol$Rules: List[Rule @unchecked], dol$OptSemicolon: Unit) => 
...
def parsetreereduction(dol$START:  org.sufrin.scalalr.SourceLocation, dol$END:  org.sufrin.scalalr.SourceLocation, n: Int): PartialFunction[List[Any], Any] = n match {
 ...
 case 7 => 
  { case trees$trees => PARSETREE("""OptPath: String = "%path" ID { $ID.toPathString }""", 7, trees$trees ) }
 case 8 => 
  { case trees$trees => PARSETREE("""OptInclude: String =  { "" }""", 8, trees$trees ) }
 case 9 => 
  { case trees$trees => PARSETREE("""OptInclude: String = "%include" CODE { $CODE }""", 9, trees$trees ) }
 case 10 => 
  { case trees$trees => PARSETREE("""OptDialects: (String,String) = "%dialect" l: ID "%scalalr" r: ID { ($l, $r) }""", 10, trees$trees ) }
 case 11 => 
  { case trees$trees => PARSETREE("""OptDialects: (String,String) = "%scalalr" l: ID "%dialect" r: ID { ($r, $l) }""", 11, trees$trees ) }
 case 12 => 
  { case trees$trees => PARSETREE("""OptDialects: (String,String) =  { ("", "") }""", 12, trees$trees ) }
 case 13 => 
  { case trees$trees => PARSETREE("""Tokens: List[TokenSpec] =  { Nil }""", 13, trees$trees ) }
 case 14 => 
  { case trees$trees => PARSETREE("""Tokens: List[TokenSpec] = TokenSpec Tokens { $TokenSpec :: $Tokens }""", 14, trees$trees ) }
 case 15 => 
  { case trees$trees => PARSETREE("""TokenSpec: TokenSpec = "%left" TypedTerminals { Left($TypedTerminals) }""", 15, trees$trees ) }
 case 16 => 
  { case trees$trees => PARSETREE("""TokenSpec: TokenSpec = "%right" TypedTerminals { Right($TypedTerminals) }""", 16, trees$trees ) }
 case 17 => 
  { case trees$trees => PARSETREE("""TokenSpec: TokenSpec = "%non" TypedTerminals { Nonassoc($TypedTerminals) }""", 17, trees$trees ) }
 case 18 => 
  { case trees$trees => PARSETREE("""TokenSpec: TokenSpec = "%token" TypedTerminals { Tokens($TypedTerminals) }""", 18, trees$trees ) }
 case 19 => 
  { case trees$trees => PARSETREE("""TypedTerminals: List[TypedTerminal] = TypedTerminal { List($TypedTerminal) }""", 19, trees$trees ) }
 case 20 => 
 ...
 case 23 => 
  { case trees$trees => PARSETREE("""TypedTerminal: TypedTerminal = ID {  TypedTerminal($ID, Untyped, $START) }""", 23, trees$trees ) }
 case 24 => 
  { case trees$trees => PARSETREE("""Tables: String =  { "lalr" }""", 24, trees$trees ) }
 case 25 => 
  { case trees$trees => PARSETREE("""Tables: String = "%tables" ID { mkTableType($ID) }""", 25, trees$trees ) }
 case 26 => 
  { case trees$trees => PARSETREE("""Rules: List[Rule] = Rule { List($Rule) }""", 26, trees$trees ) }
 case 27 => 
  { case trees$trees => PARSETREE("""Rules: List[Rule] = Rules NL Rule { $Rule :: $Rules }""", 27, trees$trees ) }
 case 28 => 
  { case trees$trees => PARSETREE("""Rule: Rule = LHS "=" OptBar RHS { Rule($LHS, $RHS, $START) }""", 28, trees$trees ) }
 case 29 => 
  { case trees$trees => PARSETREE("""OptBar: Unit = "|" {()}""", 29, trees$trees ) }
 case 30 => 
  { case trees$trees => PARSETREE("""OptBar: Unit =  {()}""", 30, trees$trees ) }
 case 31 => 
  { case trees$trees => PARSETREE("""LHS: TypedNonterminal = ID ":" Type {  TypedNonterminal($ID, $Type, $START) }""", 31, trees$trees ) }
 case 32 => 
  { case trees$trees => PARSETREE("""LHS: TypedNonterminal = ID {  TypedNonterminal($ID, Untyped, $START) }""", 32, trees$trees ) }
 case 33 => 
  { case trees$trees => PARSETREE("""RHS: List[Production] = Production { List($Production)   }""", 33, trees$trees ) }
 case 34 => 
  { case trees$trees => PARSETREE("""RHS: List[Production] = Production "|" RHS { $Production :: $RHS }""", 34, trees$trees ) }
 case 35 => 
  { case trees$trees => PARSETREE("""Production: Production = NamedFields Action Precedence { Production($NamedFields, $Action, $Precedence, $START) }""", 35, trees$trees ) }
 case 36 => 
  { case trees$trees => PARSETREE("""NamedFields: List[NamedField] = "%empty" { Nil }""", 36, trees$trees ) }
 case 37 => 
  { case trees$trees => PARSETREE("""NamedFields: List[NamedField] = NamedField { List($NamedField) }""", 37, trees$trees ) }
 case 38 => 
  { case trees$trees => PARSETREE("""NamedFields: List[NamedField] = NamedField NamedFields { $NamedField :: $NamedFields }""", 38, trees$trees ) }
 case 39 => 
  { case trees$trees => PARSETREE("""NamedField: NamedField = ID { NamedField(theName = None, fieldSymbol = $ID, $START) }""", 39, trees$trees ) }
 case 40 => 
  { case trees$trees => PARSETREE("""NamedField: NamedField = theName: ID ":" fieldSymbol: ID { NamedField(Some($theName), $fieldSymbol, $START) }""", 40, trees$trees ) }
 case 41 => 
  { case trees$trees => PARSETREE("""Action: Option[Expression] =  { None }""", 41, trees$trees ) }
 case 42 => 
  { case trees$trees => PARSETREE("""Action: Option[Expression] = CODE { Some($CODE) }""", 42, trees$trees ) }
 case 43 => 
  { case trees$trees => PARSETREE("""Precedence: Option[Terminal] =  { None }""", 43, trees$trees ) }
 case 44 => 
  { case trees$trees => PARSETREE("""Precedence: Option[Terminal] = "%prec" ID { Some(new Terminal($ID)) }""", 44, trees$trees ) }
 case 45 => 
  { case trees$trees => PARSETREE("""Type: Type = ID { Type($ID, Nil, $START) }""", 45, trees$trees ) }
 case 46 => 
  { case trees$trees => PARSETREE("""Type: Type = ID "[" Types "]" { Type($ID, $Types, $START) }""", 46, trees$trees ) }
 case 47 => 
  { case trees$trees => PARSETREE("""Type: Type = "(" Types ")" { makeTupleType($Types, $START) }""", 47, trees$trees ) }
 case 48 => 
  { case trees$trees => PARSETREE("""Type: Type = "(" ")" { Type("Unit", Nil, $START) }""", 48, trees$trees ) }
 case 49 => 
  { case trees$trees => PARSETREE("""Types: List[Type] = Type { List($Type) }""", 49, trees$trees ) }
 case 50 => 
  { case trees$trees => PARSETREE("""Types: List[Type] = Type "," Types { $Type :: $Types }""", 50, trees$trees ) }
 }
